...
optional<vector<int>> run(int final_prefix, int cur_prefix, bitset<5> flags) {
  ...
  const vector<pair<function<double(int)>, function<double(int)>>> fs = {
    {square_f, square_g},
  ...
  for (int k = 0; k < fs.size(); k++) {
    if (!flags[k]) {
      for (int i = ceil(fs[k].second(cur_prefix)); fs[k].first(i) < cur_prefix + 100; i++) {
        const int n = fs[k].first(i);
        const int cur_prefix = n % 100 * 100;
        if (cur_prefix < 1000) {
          continue;
        }
        flags[k] = true;
        const auto result = run(final_prefix, cur_prefix, flags);
        flags[k] = false;
        if (result.has_value()) {
          auto v = result.value();
          v.insert(v.begin(), n);
...
int main() {
  ...
  for (int i = ceil(triangle_g(1000)); triangle_f(i) < 10000; i++) {
    const int n = triangle_f(i);

    const int final_prefix = n / 100;
    const int cur_prefix = n % 100 * 100;
    if (cur_prefix < 1000) {
      continue;
    }
